...
# TODO: 接收AD持续输入，持续显示小球位置。（之前的位置要保留还是擦除？或渐变消除。）
...
manager.widgets.append(inpututil.TextInput(text_color=pygame.Color('orange'), cursor_color=pygame.Color('orange'), rect=pygame.Rect(362, 300, 100, 35), active=True))
manager.widgets.append(inpututil.TextInput(text_color=pygame.Color('orange'), cursor_color=pygame.Color('orange'), rect=pygame.Rect(562, 300, 100, 35)))
manager.widgets.append(inpututil.TextInput(text_color=pygame.Color('orange'), cursor_color=pygame.Color('orange'), rect=pygame.Rect(362, 365, 100, 35)))
manager.widgets.append(inpututil.TextInput(text_color=pygame.Color('orange'), cursor_color=pygame.Color('orange'), rect=pygame.Rect(562, 365, 100, 35)))
manager.widgets.append(inpututil.TextInput(text_color=pygame.Color('orange'), cursor_color=pygame.Color('orange'), rect=pygame.Rect(362, 430, 100, 35)))
manager.widgets.append(inpututil.TextInput(text_color=pygame.Color('orange'), cursor_color=pygame.Color('orange'), rect=pygame.Rect(562, 430, 100, 35)))
...
while running:
    ...
    for event in events:
        if event.type == KEYDOWN: # 监听键盘，按下ESC就退出
            ...
            if click_box_x <= x <= (click_box_x + click_box_w) and click_box_y <= y <= (click_box_y + click_box_h): # 如果点击在click_box之内
    ...
    if game_started: # 游戏已经开始
        ...
        screen.blit(start_game_string, (click_box_x, click_box_y))
